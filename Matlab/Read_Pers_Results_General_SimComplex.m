function [points, pointsVal, pers_list, red_list, bd_list] = Read_Pers_Results_General_SimComplex(dataName)
...
[~] = fread(data_fid, 1, 'uint'); % read the maximum dimension
...
pointsVal = fread(data_fid, numPoints, 'double'); % read point values
...
for i=1:maxDim
    tmp = fread(pers_fid, persSize(i)*2, 'uint');
    tmp = reshape(tmp, [2, persSize(i)]);
    pers_list{i} = tmp;
end
fclose(pers_fid);
